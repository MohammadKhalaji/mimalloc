...
int main() {
    ...
    // make sure to align the mmapped memory to a multiple of the page size:
    // MI_SEGMENT_SIZE

    char* file_in_memory = mmap(NULL, FILE_SIZE, PROT_READ | PROT_WRITE,
    ...
    bool ret = mi_manage_os_memory((void*)file_in_memory, FILE_SIZE, true, true,
                                   true, -1);
